...
class AppConfig extends ConfigBase {
  ...
  isSupport(platformName) {
    return (Support_Platforms.includes(platformName))
  }

  getOauthConfig(appConfig, platformName) {
    const treePath = OauthConfig[platformName]
    const node = this.findNode(appConfig, treePath)
  ...
  findNode(treeNode, arrayPath) {
    let node = treeNode
    for (const treePath of arrayPath) {
